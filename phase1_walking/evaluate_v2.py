...
def evaluate(
    ...
    print("\n" + "=" * 60)
    if avg_length >= 100 and avg_distance >= 1.0:
        print(" ✅ SUCCESS: 로봇이 안정적으로 걷고 있습니다!")
    elif avg_length >= 50:
        print(" ⚠️  PARTIAL: 로봇이 서 있지만 충분히 걷지 않습니다.")
    else:
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate G1 v2 agent")
    parser.add_argument(
        "--model",
        type=str,
        default="models/ppo_g1_v2/best_model",
        help="Model path (without .zip extension)",
    )
    parser.add_argument(
        "--vec-normalize",
        type=str,
        default="models/ppo_g1_v2/vec_normalize.pkl",
    ...
    if not model_path.exists() and not model_path.with_suffix(".zip").exists():
        ...
        print(f"   python -m phase1_walking.train_v2\n")
        return
