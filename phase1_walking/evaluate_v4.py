"""Evaluate trained G1 v4 agent (Exponential Reward + Stability)."""
import argparse
...
def evaluate(
    ...
    """평가 실행."""
    print("=" * 60)
    print(" 🎬 G1 v4 평가 (Exponential Reward + Stability)")
    print("=" * 60)
    print(f"\n📦 모델: {model_path}")
    ...
    elif avg_length >= 150 and avg_distance >= 2.0:
        print(" 🎉 GOOD: 상당한 진전이 있습니다!")
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate G1 v4 agent")
    parser.add_argument(
        "--model",
        type=str,
        default="models/ppo_g1_v4/best_model",
        help="Model path (without .zip extension)",
    )
    parser.add_argument(
        "--vec-normalize",
        type=str,
        default="models/ppo_g1_v4/vec_normalize.pkl",
    ...
    if not model_path.exists() and not model_path.with_suffix(".zip").exists():
        ...
        print(f"   python -m phase1_walking.train_v4\n")
        return
