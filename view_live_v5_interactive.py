"""학습된 G1 v5를 MuJoCo Interactive Viewer로 실시간 시각화.

V5: Linear Forward + Stability Termination
Run: ./venv/bin/mjpython view_live_v5_interactive.py
"""
import numpy as np
import mujoco
...
def main():
    print("=" * 60)
    print(" G1 v5 - Interactive Viewer")
    print(" (Linear Forward + Stability Termination)")
    print("=" * 60)

    model_path = Path("models/ppo_g1_v5/best_model")
    vec_normalize_path = Path("models/ppo_g1_v5/vec_normalize.pkl")

    if not model_path.with_suffix(".zip").exists():
        print("Model not found. Run training first:")
        print("   python -m phase1_walking.train_v5")
        return
